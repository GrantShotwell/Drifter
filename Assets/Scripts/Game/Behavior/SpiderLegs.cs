...
public class SpiderLegs : MonoBehaviour {
    ...
    private void Update() {
        ...
        foreach(Leg leg in legs) {
            ...
            if((!range.Contains(legAngle)) || legDistance > leg.length || leg.temporaryFooting) {
                ...
                deltaAngle = - angleSeparation * Geometry.LinearDirection(start - end);
    ...
    float MeasureAngle(Leg leg) {
        return Geometry.NormalizeDegree1(Vector2.SignedAngle(leg.parent.transform.position, leg.foot.transform.position));
    }
    #endregion
}
