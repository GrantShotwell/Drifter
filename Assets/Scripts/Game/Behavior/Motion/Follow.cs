...
public class Follow : MonoBehaviour {
    ...
    private void FixedUpdate() {
        ...
        if(maximumDistance >= distance && distance > minimumDistance && speed < maxSpeed) {
            ...
            if(ignoreX != ignoreY) {
                if(!ignoreX) F = new Vector2(force * Geometry.LinearDirection(target.transform.position.x - transform.position.x), 0);
                if(!ignoreY) F = new Vector2(0, force * Geometry.LinearDirection(target.transform.position.y - transform.position.y));
            }
